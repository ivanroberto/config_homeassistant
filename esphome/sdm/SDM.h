...
float SDM::readVal(uint16_t reg, uint8_t node) {
  ...
  ESP_LOGD("SDM", "BufferTX: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x", sdmarr[0], sdmarr[1], sdmarr[2], sdmarr[3], sdmarr[4], sdmarr[5], sdmarr[6], sdmarr[7]);
...
//  delay(2);                                                                     //fix for issue (nan reading) by sjfaustino: https://github.com/reaper7/SDM_Energy_Meter/issues/7#issuecomment-272111524
  ...
  while (sdmSer.available() < 6) {
    if (resptime < millis()) {
      readErr = SDM_ERR_TIMEOUT;                                                //err debug (4)
      ESP_LOGD("SDM", "Timeout reading bytes");
      break;
    }
    yield();
  }
  ...
  if (readErr == SDM_ERR_NO_ERROR) {                                            //if no timeout...
    ...
    if(sdmSer.available() >= 6) {
      int address, function, dataLen;
      uint8_t dataBuffer[16];
      address = sdmSer.read();
      function = sdmSer.read();
      dataLen = sdmSer.read();
    
      ESP_LOGD("SDM", "address: 0x%x, function: 0x%x, dataLen: %d Bytes", address, function, dataLen);
    
      for(int n=0; n<dataLen*2; n++) {
        dataBuffer[n] = sdmSer.read();
        ESP_LOGD("SDM", "Loop read dataBuffer: 0x%x", dataBuffer[n]);
      }
